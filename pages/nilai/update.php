...
if ( isset($_GET["id"]) )
{
  // Get id
  $id = $_GET["id"];
  $SQL = "SELECT mahasiswa.nama, nilai.id_mhs, nilai.mata_kuliah, nilai.id, nilai.nilai, nilai.huruf FROM mahasiswa INNER JOIN nilai ON mahasiswa.id = nilai.id_mhs AND nilai.id = $id";
  $rows = [];
...
if ( isset($_POST["update"]) )
{
  ...
  $id = $_POST["id"];
  $nilai = $_POST["nilai"];
  $huruf = createGrade($nilai);
  
  $SQL = "UPDATE nilai SET nilai = '$nilai', huruf = '$huruf' WHERE id = $id";
  $query_results = mysqli_query($conn, $SQL);
...
<html lang="en">
  ...
  <nav class="navbar navbar-light bg-success">
    <div class="container-fluid">
      <a class="navbar-brand text-white" href="#">Penilaian Mahasiswa</a>
    </div>
  </nav>
...
echo <<<EOT
      ...
      <a href="../nilai" class="btn btn-warning">Kembali</a>
